const solution = () => {
  ...
  const adjArr = Array.from(Array(N + 1), () => Array(N + 1).fill(0));
  for (let i = 1; i <= M; i++) {
  ...
  for (let start = 1; start <= N; start++) {
    if (start !== X) {
      const distance = dijkstra(start, adjArr, X);
  ...
  for (let destination = 1; destination <= N; destination++) {
    if (destination !== X) {
      const distance = dijkstra(X, adjArr, destination);
...
const dijkstra = (start, adjArr, destination) => {
  const discovered = Array(adjArr.length).fill(Number.MAX_SAFE_INTEGER);
  discovered[start] = 0;
  const heap = new Heap();
  heap.add([start, 0]);
  ...
  while (heap.len !== 0) {
    ...
    for (let next = 1; next < adjArr.length; next++) {
      if (adjArr[cityNum][next] === 0) {
        continue;
      }
      const nextDistance = distance + adjArr[cityNum][next];
